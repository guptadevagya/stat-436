...
visualizing the diamonds dataset below are TRUE or FALSE. [0.75 points each]
    ...
    variables, you can use:\newline $~$ \hspace{10mm} `ggplot(diamonds) + geom_point(aes(price, depth))`
    
    b. TRUE FALSE To visualize the relationship between x and caret variables with
    cut encoded by color, you can use:\newline $~$ \hspace{10mm} `ggplot(diamonds) + geom_point(aes(price, depth)) + scale_color_manual(cut)`.
    ...
    clarity:\newline $~$ \hspace{10mm} `ggplot(diamonds) + geom_point(aes(caret, price)) + facet_wrap(~ clarity)`.\newline To sort facets by price, you could modify the `geom_point` layer to `geom_point(aes(carat, reorder(price)))`.
...
studies/outside knowledge, or pick one from these suggested collections: [TidyTuesday](https://github.com/rfordatascience/tidytuesday#datasets), [Kaggle](https://www.kaggle.com/datasets), [Google Dataset Search](https://datasetsearch.research.google.com/), [AWS Data Registry](https://registry.opendata.aws/), [Wisconsin DNR](https://data-wi-dnr.opendata.arcgis.com/), [IPUMS Census Data](https://www.ipums.org/), [538](https://github.com/fivethirtyeight/data/)
    ...
    **With your chosen dataset, use `patchwork` to create a compound figure with at least three individual visualizations.**
    ...
    The goal of here is to create something you would be proud to show someone
